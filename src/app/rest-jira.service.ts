...
export class RestJiraService {
  ...
  this.opts.headers = this.headers;
    ...
    return this.http.get('http://localhost:4200/rest/api/latest/issue/' + issue + '/comment', this.opts).map(res => res.json());
  }
  ...
  autenticar() {
    ...
    this.headers.set('User-Agent', 'Portal TPRIME - Desenvolvimento PRIME');
    this.opts.headers = this.headers;
    return this.http.get('http://localhost:4200/rest/auth/latest/session', this.opts)
    .map(res => res.json());
  }
